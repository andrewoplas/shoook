...
import com.shoook.entity.Admin;
...
public class AuthRepository {
	...
	public UserLoginResponse loginAdmin(EntityManager em, UserLogin user) {
		...
		query.setParameter("email_address", user.getEmailAddress().trim());
		...
		if(results.size() == 1) {
			...
			ulr.setId(admin.getId());
			ulr.setFirstName(admin.getFirstName());
			ulr.setLastName(admin.getLastName());
			ulr.setRole(ADMIN);
		}
